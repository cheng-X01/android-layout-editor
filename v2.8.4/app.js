...
function drawGridNumbers() {
  ...
  for (let x = startX; x <= scrollX + w; x += step) {
    const screenX = x - scrollX;
    if (screenX >= 0 && screenX <= w) {
      ctx.fillText(Math.round(x / z), screenX + 2, 2);
    }
  }

  // 左边数字（Y轴）
  const startY = Math.floor(scrollY / step) * step;
  for (let y = startY; y <= scrollY + h; y += step) {
    const screenY = y - scrollY;
    if (screenY >= 0 && screenY <= h) {
      ctx.fillText(Math.round(y / z), 2, screenY + 2);
    }
  }
}
...
// 设计窗口全屏（隐藏左右面板和主工具栏，保留设计窗+代码+浮动工具条）
