...
function drawGridNumbers() {
  ...
  for (let x = startX; x <= scrollX + w + step; x += step) {
    const screenX = x - scrollX;
    if (screenX >= -step && screenX <= w + step) {
      // 显示实际像素值（考虑缩放）
      const realX = Math.round(x / z);
      if (realX % 100 === 0) {
        ctx.fillText(realX, screenX + 4, 4);
      }
    }
  }
  ...
  for (let y = startY; y <= scrollY + h + step; y += step) {
    const screenY = y - scrollY;
    if (screenY >= -step && screenY <= h + step) {
      // 显示实际像素值（考虑缩放）
      const realY = Math.round(y / z);
      if (realY % 100 === 0) {
        ctx.fillText(realY, 4, screenY + 4);
      }
    }
  }
}
