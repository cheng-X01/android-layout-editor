...
function drawRuler(axis) {
  const canvas = axis === 'h' ? $('rulerHCanvas') : $('rulerVCanvas');
  const container = $('canvasContainer');
  if (!canvas || !container) return;

  const rect = container.getBoundingClientRect();
  const isH = axis === 'h';
  const size = isH ? rect.width - 30 : rect.height - 30;
  const scroll = isH ? container.scrollLeft : container.scrollTop;
  const dpr = window.devicePixelRatio || 1;
  const RULER_SIZE = 30;

  canvas.width = (isH ? size : RULER_SIZE) * dpr;
  canvas.height = (isH ? RULER_SIZE : size) * dpr;
  canvas.style.width = (isH ? size : RULER_SIZE) + 'px';
  canvas.style.height = (isH ? RULER_SIZE : size) + 'px';

  const ctx = canvas.getContext('2d');
  ctx.scale(dpr, dpr);
  ctx.clearRect(0, 0, isH ? size : RULER_SIZE, isH ? RULER_SIZE : size);

  const step = 50;
  const startPos = Math.floor(scroll / step) * step;
  const endPos = startPos + size + step;

  ctx.fillStyle = '#888';
  ctx.font = '9px monospace';

  for (let pos = startPos; pos <= endPos; pos += step) {
    const isMajor = pos % 100 === 0;
    const tickLen = isMajor ? 12 : 7;
    const coord = pos - scroll;
    ...
    if (isH) {
      ctx.moveTo(coord, RULER_SIZE - tickLen);
      ctx.lineTo(coord, RULER_SIZE);
      ctx.stroke();
      if (isMajor && pos > 0) {
        ctx.textAlign = 'center';
        ctx.fillText(pos, coord, RULER_SIZE - tickLen - 2);
      }
    } else {
      ctx.moveTo(RULER_SIZE - tickLen, coord);
      ctx.lineTo(RULER_SIZE, coord);
      ctx.stroke();
      if (isMajor && pos > 0) {
        ctx.save();
        ctx.translate(RULER_SIZE - tickLen - 2, coord);
