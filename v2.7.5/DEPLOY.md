...
| 资源 | URL | 用途 | 离线备份建议 |
|------|-----|------|-------------|
| JSZip | https://cdnjs.cloudflare.com/... | ZIP导出 | 下载到本地 js/ 目录 |

### 离线部署（无外部依赖）

1. 下载 JSZip: `curl -o jszip.min.js https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js`
2. 修改 `index.html` 第8行：
   ```html
   <!-- 原 -->
...
| 无法导出ZIP | JSZip 加载失败 | 检查网络连接或改用本地 JSZip |
